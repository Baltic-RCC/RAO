...
def convert(list_of_nc_paths_or_file_objects):
    rao_data = pandas.read_RDF(list_of_nc_paths_or_file_objects)

    for key, value in rao_data.types_dict().items():
        logger.info(f"Loaded {value} {key}")
...
if __name__ == "__main__":
    ...
    crac = convert([PATH_ASSESSED_ELEMENTS, PATH_CONTINGENCIES, PATH_REMEDIAL_ACTIONS])

    with open(EXPORT_CRAC_NAME, "w") as file_object:
        json.dump(crac, file_object, sort_keys=False, indent=2)
        logger.info(f"Created {EXPORT_CRAC_NAME}")
